        ...
        if path and path.exists() and yaml:
            ...
            if isinstance(loaded, dict):
                ...
                if isinstance(buckets_override, dict):
                    buckets_data = cast(
                        Dict[str, Any], config_data.setdefault("buckets", {})
                    )
                    buckets_data.update(buckets_override)
                if isinstance(weights_override, dict):
                    weights_data = cast(
                        Dict[str, Any], config_data.setdefault("weights", {})
                    )
        ...
        if bucket.dir_keywords:
            for keyword in bucket.dir_keywords:
                if keyword in path_lower or (
                    record.dir_hint and keyword in record.dir_hint
                ):
...
def derive_project_label(doc_ids: List[str], fallback: str) -> str:
    ...
    if usable:
        ...
        counts = Counter(
            part for part in flattened if part and part not in SKIP_LABEL_SEGMENTS
        )
        if counts:
...
def simple_cluster(
    ...
    for label, doc_ids in groups.items():
        bucket_counts = Counter(
            entry.get("bucket", "tmp")
            for entry in items
            if entry.get("path") in doc_ids
        )
        projects.append(
            {
                "project_id": label,
...
def pack_items_adaptively(items: Sequence[Dict[str, Any]], max_bytes: int = 700 * 1024):
    """예산 기반으로 아이템을 배치로 나누기"""
    batch, size = [], 0

    for item in items:
        # 최소화된 아이템 직렬화
        line = serialize_min_item(item) + "\n"
        item_bytes = len(line.encode("utf-8"))

        # 배치 크기 초과 시 현재 배치 반환
        if size + item_bytes > max_bytes and batch:
            yield batch
            batch, size = [], 0
...
def send_with_retry(
    ...
    while True:
        ...
        try:
            click.echo(
                f"[send_with_retry] Attempt {attempt} with {len(current_batch)} items"
            )
            return gpt_cluster_single_batch(current_batch, safe_map_path, model_env_var)

        except Exception as e:
            error_msg = str(e)

            # HTTP 400 (페이로드 너무 큼) - 자동 재분할
            if "400" in error_msg or "Bad Request" in error_msg:
                if len(current_batch) <= min_batch_size:
                    click.echo(
                        f"[send_with_retry] Cannot split further (size: {len(current_batch)}), using fallback"
                    )
                    # 더 이상 분할할 수 없으면 폴백 처리
                    return create_fallback_projects(current_batch)

                # 배치를 반으로 나누기
                mid = len(current_batch) // 2
                click.echo(
                    f"[send_with_retry] Splitting batch from {len(current_batch)} to {mid} items due to 400 error"
                )

                # 두 개의 하위 배치로 재귀 처리
                result1 = send_with_retry(
                    current_batch[:mid], safe_map_path, model_env_var, min_batch_size
                )
                result2 = send_with_retry(
                    current_batch[mid:], safe_map_path, model_env_var, min_batch_size
                )

                # 결과 병합
                return {
                    "projects": result1.get("projects", [])
                    + result2.get("projects", [])
                }

            # HTTP 429 (레이트리밋) - 백오프
            elif "429" in error_msg or "Too Many Requests" in error_msg:
                backoff_time = min(2**attempt, 60)  # 최대 60초
                click.echo(
                    f"[send_with_retry] Rate limited, backing off for {backoff_time}s"
                )
                import time
            ...
            # 기타 오류 - 재시도
                ...
                if attempt >= 3:  # 최대 3회 재시도
                    click.echo(f"[send_with_retry] Max retries reached, using fallback")
                    return create_fallback_projects(current_batch)

                backoff_time = min(2**attempt, 10)
                click.echo(
                    f"[send_with_retry] Error on attempt {attempt}: {e}, retrying in {backoff_time}s"
                )
                import time
...
def create_fallback_projects(items: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    """배치 실패 시 폴백 프로젝트 생성"""
    projects = []
    for item in items:
        projects.append(
            {
                "project_id": item.get("name", "unknown"),
                "project_label": item.get("name", "unknown"),
                "doc_ids": [item.get("path", "")],
                "role_bucket_map": {},
                "confidence": 0.5,
                "reasons": ["fallback"],
            }
...
def gpt_cluster_batch(
    items: Sequence[Dict[str, Any]],
    safe_map_path: str,
    model_env_var: str,
    batch_size: int,
) -> Dict[str, Any]:
    """예산 기반 배치로 GPT 클러스터링 수행"""
    MAX_PAYLOAD_BYTES = 700 * 1024  # 700KB 제한
    MIN_BATCH_SIZE = 5

    click.echo(
        f"[gpt_cluster_batch] Processing {len(items)} items with budget-based batching (max {MAX_PAYLOAD_BYTES} bytes)"
    )

    all_projects = []
    batches = list(pack_items_adaptively(items, MAX_PAYLOAD_BYTES))
    total_batches = len(batches)

    click.echo(f"[gpt_cluster_batch] Created {total_batches} budget-based batches")

    for i, batch_items in enumerate(batches):
        batch_num = i + 1
        batch_size_bytes = sum(
            len(serialize_min_item(item).encode("utf-8")) for item in batch_items
        )
        click.echo(
            f"[gpt_cluster_batch] Processing batch {batch_num}/{total_batches} ({len(batch_items)} items, ~{batch_size_bytes} bytes)"
        )

        try:
            # 자동 재분할 및 재시도 로직
            batch_result = send_with_retry(
                batch_items, safe_map_path, model_env_var, MIN_BATCH_SIZE
            )
            all_projects.extend(batch_result.get("projects", []))
            click.echo(
                f"[gpt_cluster_batch] Batch {batch_num} completed: {len(batch_result.get('projects', []))} projects"
            )
        except Exception as e:
            click.echo(f"[gpt_cluster_batch] Batch {batch_num} failed: {e}")
            # 배치 실패 시 해당 배치의 아이템들을 개별 프로젝트로 처리
                ...
                all_projects.append(
                    {
                        ...
                        "role_bucket_map": {},
                        "confidence": 0.5,
                        "reasons": ["batch_fallback"],
                    }
                )

    click.echo(
        f"[gpt_cluster_batch] All batches completed: {len(all_projects)} total projects"
    )
    return {"projects": all_projects}


def gpt_cluster_single_batch(
    items: Sequence[Dict[str, Any]],
    safe_map_path: str,
    model_env_var: str,
) -> Dict[str, Any]:
    """단일 배치에 대한 GPT 클러스터링 (NDJSON + gzip 지원)"""
    ...
    def build_gpt_payload_ndjson(items: Sequence[Dict[str, Any]]) -> str:
        """NDJSON 형태로 페이로드 생성"""
        ...
        for it in items:
            ...
            min_item = {
                "safe_id": safe_id,
                "name": it.get("name", "")[:120],
                "bucket": it.get("bucket", ""),
                "hint": it.get("dir_hint", ""),
                "sample": extract_sample_text(it, limit_chars=400),  # 더 작은 샘플
            }
            ndjson_lines.append(json.dumps(min_item, ensure_ascii=False))

        # safe_map 업데이트
        json.dump(
            safe_map,
            open(safe_map_path, "w", encoding="utf-8"),
            ensure_ascii=False,
            indent=2,
        )
    ...
    # NDJSON 페이로드 생성
    ndjson_content = build_gpt_payload_ndjson(items)
    ...
    # gzip 압축
    payload_bytes = json.dumps(api_payload, ensure_ascii=False).encode("utf-8")
    compressed_payload = gzip.compress(payload_bytes)

    click.echo(
        f"[gpt_cluster_single_batch] Payload: {len(payload_bytes)} bytes -> {len(compressed_payload)} bytes (gzipped)"
    )
...
# GPT 클러스터링 토큰 기반 배치 처리 유틸리티 import
from gpt_cluster import (
    SimpleEncoder,
    serialize_min_item,
    pack_items_by_tokens,
    send_with_retry_new,
    bounded_gather,
    make_api_call_fn,
)
...
def gpt_cluster_new(
    items: Sequence[Dict[str, Any]],
    safe_map_path: str,
    model_env_var: str = "OPENAI_API_KEY",
) -> Dict[str, Any]:
    """새로운 토큰 기반 GPT 클러스터링"""
    # 1) 토큰 예산으로 선배치
    encoder = SimpleEncoder(avg_chars_per_token=4)
    batches = pack_items_by_tokens(items, max_tokens=1500, encoder=encoder)
    ...
    for batch_items in batches:
        try:
            batch_result = send_with_retry_new(
                batch_items, api_call=api_call, min_batch_size=1
            )
            click.echo(f"[gpt_cluster_new] Batch result type: {type(batch_result)}")
            click.echo(
                f"[gpt_cluster_new] Batch result length: {len(batch_result) if hasattr(batch_result, '__len__') else 'N/A'}"
            )

            # 결과가 리스트인지 확인
            if not isinstance(batch_result, list):
                click.echo(
                    f"[gpt_cluster_new] Unexpected result type: {type(batch_result)}"
                )
                batch_result = []
            ...
            for item in batch_result:
                if isinstance(item, dict):
                    project = {
                        "project_id": item.get("label", "unknown"),
                        "project_label": item.get("label", "unknown"),
                        "doc_ids": [item.get("id", "")],
                        "role_bucket_map": {},
                        "confidence": 0.8,
                        "reasons": ["gpt_cluster_new"],
                    }
                    all_projects.append(project)
                else:
                    click.echo(f"[gpt_cluster_new] Unexpected item type: {type(item)}")
        except Exception as e:
            msg = str(e)
            # 401 오류는 즉시 상위로 전파
            if "401" in msg or "Unauthorized" in msg:
                click.echo(f"[gpt_cluster_new] 401 Unauthorized detected: {msg}")
                raise
            click.echo(f"[gpt_cluster_new] Batch failed: {e}")
            # 실패한 배치의 아이템들을 개별 프로젝트로 처리
            for item in batch_items:
                project = {
                    "project_id": item.get("name", "unknown"),
                    "project_label": item.get("name", "unknown"),
...
def gpt_cluster(
    ...
    """KR: GPT 기반 프로젝트 군집화 (Chat Completions API + SDK). EN: GPT-powered project clustering using Chat Completions API."""

    # 일단 기존 urllib 방식 유지 (SDK 의존성 문제 해결 후 교체)
    import urllib.request
    import urllib.parse

    click.echo(f"[gpt_cluster] Starting GPT clustering with {len(items)} items")

    api_key = os.environ.get(model_env_var)
    if not api_key:
        click.echo(f"[gpt_cluster] ERROR: Environment variable {model_env_var} not set")
        raise RuntimeError(f"Environment variable {model_env_var} not set")

    click.echo(f"[gpt_cluster] API key found: {api_key[:10]}...")

    # 대용량 데이터 처리: 배치 크기 제한
    MAX_ITEMS_PER_BATCH = 100  # 한 번에 처리할 최대 아이템 수
    if len(items) > MAX_ITEMS_PER_BATCH:
        click.echo(
            f"[gpt_cluster] Large dataset detected ({len(items)} items). Using batch processing with max {MAX_ITEMS_PER_BATCH} items per batch."
        )
        return gpt_cluster_batch(
            items, safe_map_path, model_env_var, MAX_ITEMS_PER_BATCH
        )

    def build_gpt_payload(items: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
        click.echo(f"[gpt_cluster] Building payload from {len(items)} items")
        safe_map = json.load(open(safe_map_path, encoding="utf-8"))
        safe_items = []
        for it in items:
            if "path" not in it:
        ...
        click.echo(f"[gpt_cluster] Created {len(safe_items)} safe items")
        json.dump(
            safe_map,
            open(safe_map_path, "w", encoding="utf-8"),
            ensure_ascii=False,
    ...
    payload = build_gpt_payload(items)
    click.echo(f"[gpt_cluster] Payload size: {len(json.dumps(payload))} characters")
    ...
    click.echo("[gpt_cluster] Sending request to OpenAI API...")
    req = urllib.request.Request(
        "https://api.openai.com/v1/chat/completions",
    ...
    try:
        click.echo("[gpt_cluster] Waiting for API response (timeout: 120s)...")
        with urllib.request.urlopen(req, timeout=120) as resp:  # 타임아웃 2분으로 증가
            click.echo(f"[gpt_cluster] API response received: {resp.status}")
            out = json.loads(resp.read().decode("utf-8"))
        content = out["choices"][0]["message"]["content"]
        click.echo(f"[gpt_cluster] Response content length: {len(content)} characters")
        data = json.loads(content)
        click.echo(
            f"[gpt_cluster] Parsed JSON with {len(data.get('projects', []))} projects"
        )
    except Exception as e:
        click.echo(f"[gpt_cluster] API call failed: {e}")
        # 401은 재시도 불가 → 즉시 상위로 전파하여 폴백 유도
        if "401" in str(e) or "Unauthorized" in str(e):
            raise RuntimeError(
                "401 Unauthorized from OpenAI; check API key / project access"
            ) from e
        raise

    # ★ 여기서 safe_id → path 역매핑
    click.echo("[gpt_cluster] Converting safe_ids back to file paths...")
    safe_map = json.load(open(safe_map_path, encoding="utf-8"))
    projects = []
    for i, p in enumerate(data.get("projects", [])):
        ...
        if not paths:
            click.echo(f"[gpt_cluster] Skipping project {i+1}: no valid paths found")
            continue
        raw_seed = p.get("project_label") or p.get("project_id") or "misc"
        label = derive_project_label(paths, raw_seed)
        normalized_seed = normalize_label(raw_seed)
        ...
        click.echo(
            f"[gpt_cluster] Project {i+1}: {label} ({len(paths)} files, confidence: {p.get('confidence', 0.7):.2f})"
        )

    click.echo(
        f"[gpt_cluster] GPT clustering completed: {len(projects)} projects generated"
    )
    return {"projects": projects}
...
def versioned_destination(
    dst_dir: Path, filename: str, hash7: str, conflict: str
) -> Path:
...
def emit_console_table(
    title: str, rows: Sequence[Sequence[Any]]
...
@click.option(
    "--safe-map", "safe_map_path", default=str(DEFAULT_SAFE_MAP), show_default=True
)
...
@click.option("--include", "include_globs", multiple=True, help="포함 글롭 패턴")
@click.option("--exclude", "exclude_globs", multiple=True, help="제외 글롭 패턴")
@click.option("--max-depth", type=int, default=None, help="최대 재귀 깊이")
@click.option("--overall-timeout", type=float, default=None, help="전체 타임아웃(초)")
@click.option("--per-batch-timeout", type=float, default=None, help="배치 타임아웃(초)")
def scan(
    paths: Sequence[str],
    sample_bytes: int,
    ...
    per_batch_timeout: Optional[float],
) -> None:
    ...
    stats = stream_paths_to_files(
        ...
        output_path=Path(emit),
        safe_map_path=Path(safe_map_path),
    ...
    click.echo(f"[scan] {stats.processed} items -> {emit}")
...
def cluster(
    scores: str, emit: str, project_mode: str, safe_map_path: str, hints: str
) -> None:
    ...
    if project_mode.lower() == "gpt":
        # GPT 모드 실행 - 401은 즉시 폴백, 기타 오류만 재시도
        max_retries = 3
        for attempt in range(max_retries):
            try:
                click.echo(f"[cluster] GPT mode attempt {attempt + 1}/{max_retries}")
                output = gpt_cluster_new(items, safe_map_path=safe_map_path)
                click.echo(f"[cluster] GPT mode succeeded on attempt {attempt + 1}")
                break
            except Exception as exc:
                msg = str(exc)
                # 401은 재시도 불가 → 즉시 로컬 폴백
                if "401" in msg or "Unauthorized" in msg:
                    click.echo(
                        "[cluster] GPT mode fatal: 401 Unauthorized → no retry; fallback to local"
                    )
...
@click.option(
    "--mode", default="move", type=click.Choice(["move", "copy"]), show_default=True
)
@click.option(
    "--conflict",
    default="version",
...
def organize(
    ...
    click.echo(
        f"[organize] moves={total_moves} -> {target_root} (journal: {journal_path})"
    )
...
def report(journal: str, out: str) -> None:
    ...
    project_table = build_table(
        "프로젝트 분포 / Project Distribution", summary["projects"]
    )
    ...
    with csv_path.open("w", encoding="utf-8", newline="") as handle:
        ...
        for key, value in sorted(
            summary["projects"].items(), key=lambda kv: (-kv[1], kv[0])
        ):
