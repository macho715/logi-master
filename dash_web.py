...
def render_sidebar(config: PipelineConfig, data: PipelineData) -> SidebarState:
    ...
    with st.sidebar:
        ...
        run_clicked = st.button(
            "🚀 파이프라인 실행 · Run pipeline", type="primary", use_container_width=True
        )
        clear_cache = st.button("🧹 캐시 삭제 · Clear cache", use_container_width=True)
...
def render_file_table(data: PipelineData, filters: SidebarFilters) -> None:
    ...
    if filters.query:
        ...
        mask = path_series.str.contains(
            filters.query, case=False, na=False
        ) | name_series.str.contains(filters.query, case=False, na=False)
        df = df[mask]
